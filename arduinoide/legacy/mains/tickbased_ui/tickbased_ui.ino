...
// base configuration
#define HALL_PULSE_PER_TURN 9  // how often the hall-sensor sends a pulse per turn; a hardware constant; see hall-sensor specification or tests/hallsensorpulses
...
static uint8_t led_2_pos = 3;
static uint8_t led_2_cooldown = 2;
static uint8_t led_3_pos = 7;
static uint8_t led_3_cooldown = 4;
...
void loop() {
    if (Serial.available() > 0) {
      ...
      // send message(s) back to check
      //Serial.println(key);
      //Serial.println(val);
      // process command
      if (key.equals("led_1_cooldown")) led_1_cooldown = val;
      else if (key.equals("led_2_cooldown")) led_2_cooldown = val;
      else if (key.equals("led_3_cooldown")) led_3_cooldown = val;
      else if (key.equals("led_1_pos")) led_1_pos = val;
      else if (key.equals("led_2_pos")) led_2_pos = val;
      else if (key.equals("led_3_pos")) led_3_pos = val;
      else if (key.equals("motor_pulse_length")) analogWrite(PIN_MOTOR, val);
  }
